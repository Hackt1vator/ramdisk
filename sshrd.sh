...
ERR_HANDLER () {
    ...
    osascript -e 'display dialog "An error occurred!" with title "Hackt1vator Unlock"'
    rm -rf work 12rd
...
check_internet_connection() {
    if curl --output /dev/null --silent --head --max-time $timeout "$url"; then
        ...
        osascript -e 'display dialog "Check your internet connection and try again!" with title "Error"'
        exit
    fi
}
...
check_internet_connection
        ...
        if [ "$device_count" -ge 2 ]; then
            ...
            osascript -e 'display dialog "Please attach only one device!" with title "Hackt1vator Unlock"'
            kill -30 0
            exit 1
        fi
        ...
        case "$device_mode" in
            normal)
                ...
                osascript -e 'display dialog "Your device is in normal mode. Press ok to put the device into recovery mode!" with title "Hackt1vator Unlock"'
                ...
                osascript -e 'display dialog "Your device is now booting into recovery mode. Please put it into dfu! Press ok when the device is in dfu!" with title "Hackt1vator Unlock"'
                

                
                ;;
            recovery)
                osascript -e 'display dialog "Your device is in recovery mode. Please put it into dfu! Press ok when the device is in dfu!" with title "Hackt1vator Unlock"'

                ;;
            dfu)
            
                  osascript -e 'display dialog "Your device is in dfu mode! Press ok to create/boot your ramdisk!" with title "Hackt1vator Unlock"'

                ;;
            *)
                  osascript -e 'display dialog "No device connected!" with title "Hackt1vator Unlock"'
                  exit
                ;;
        esac
fi
...
if [ "$1" = 'boot' ]; then
    if [ ! -e sshramdisk/iBSS.img4 ]; then
        echo "[-] Please create an SSH ramdisk first!"
        osascript -e 'display dialog "Please create an ramdisk first!" with title "Hackt1vator Unlock"'
        exit
    fi
    ...
    osascript -e 'display dialog "Ramdisk successfully booted!" with title "Hackt1vator Unlock"'
    exit
fi
...
osascript -e 'display dialog "Ramdisk successfully created!" with title "Hackt1vator Unlock"'
